...
import sys
# sys.path.append("../")
# NOTE (Albert): use relative import
...
class ReformerSelfAttn(nn.Module):
    """ Self attention Layer"""
    def __init__(self, in_channels, eps=1e-12):
        super(ReformerSelfAttn, self).__init__()
    ...
    def __init__(self, config):
        ...
        for i, layer in enumerate(config.layers):
            if i == config.attention_layer_position:
                layers.append(ReformerSelfAttn(ch*layer[1], eps=config.eps))
...
class ReformerBigGAN(nn.Module):
    """BigGAN Generator."""
    ...
    def __init__(self, config):
        super(ReformerBigGAN, self).__init__()
        self.config = config
