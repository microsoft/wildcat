...
class PerformerSelfAttn(nn.Module):
    """ Self attention Layer"""
    def __init__(self, in_channels, eps=1e-12):
        super(PerformerSelfAttn, self).__init__()
    ...
    def __init__(self, config):
        ...
        for i, layer in enumerate(config.layers):
            if i == config.attention_layer_position:
                layers.append(PerformerSelfAttn(ch*layer[1], eps=config.eps))
...
class PerformerBigGAN(nn.Module):
    """BigGAN Generator."""
    ...
    def __init__(self, config):
        super(PerformerBigGAN, self).__init__()
        self.config = config
