...
import sys
sys.path.append("/home/ih244/workspace/kde/RobustAttention")
from .AttentionEstimator.CosineHamming import CosineHammingAttention
...
class FastSelfAttn(nn.Module):
    ...
    def __init__(self, in_channels, eps=1e-12):
        super(FastSelfAttn, self).__init__()
    ...
    def __init__(self, config):
        ...
        for i, layer in enumerate(config.layers):
            if i == config.attention_layer_position:
                layers.append(FastSelfAttn(ch*layer[1], eps=config.eps))
...
class FastBigGAN(nn.Module):
    ...
    def __init__(self, config):
        super(FastBigGAN, self).__init__()
        self.config = config
