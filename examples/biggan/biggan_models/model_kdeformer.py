...
# import sys
# sys.path.append('examples/biggan')
...
class KDEformerSelfAttn(nn.Module):
    """ Self attention Layer"""
    def __init__(self, in_channels, eps=1e-12):
        super(KDEformerSelfAttn, self).__init__()
    ...
    def fastformer(self, query, key, value):
        ...
        try:
            ...
            att_ham = torch.nan_to_num(att_ham, nan=0.0, posinf=1e20, neginf=-1e20)
    ...
    def __init__(self, config):
        ...
        for i, layer in enumerate(config.layers):
            if i == config.attention_layer_position:
                layers.append(KDEformerSelfAttn(ch*layer[1], eps=config.eps))
...
class KDEformerBigGAN(nn.Module):
    """BigGAN Generator."""
    ...
    def __init__(self, config):
        super(KDEformerBigGAN, self).__init__()
        self.config = config
