"""
Compute average maximum query and key norms in the sequence up to token n
for Qwen2.5-7B on the RULER dataset.
...
#DATASET_CONFIG = "16384"   
DATASET_NAME = "Xnhyacinth/LongBench"
DATASET_CONFIG = "qasper_e"       # context length bucket in the ruler dataset
...
def make_q_hook(idx: int):
    def hook(module, input, output):
        ...
        # layer_q[idx] = output.detach().float().reshape(b, s, num_heads, head_dim).transpose(1, 2)
        layer_q[idx] = output.detach().cpu().reshape(b, s, num_heads, head_dim).transpose(1, 2)
        #layer_q[idx] = output.detach().cpu()
    return hook
...
def make_k_hook(idx: int):
    def hook(module, input, output):
        ...
        #layer_k[idx] = output.detach().float().reshape(b, s, num_kv_heads, head_dim).transpose(1, 2)
        layer_k[idx] = output.detach().cpu().reshape(b, s, num_kv_heads, head_dim).transpose(1, 2)
        #layer_k[idx] = output.detach().cpu()
    return hook
...
for ctx_idx, context in enumerate(long_contexts):
    ...
    for n in POWERS_OF_TWO:
        ...
        for lq, lk in zip(layer_q, layer_k):
            ...
            g = num_heads // num_kv_heads  # e.g. 28 // 4 = 7
            ...
            # q_layer_vals.append(q_scale.detach())
            # k_layer_vals.append(k_scale.detach())
            gamma_vals.append(gamma.detach().cpu())

        # mean over layers
        # q_norms_all[n].append(float(np.mean(q_layer_vals)))
        # k_norms_all[n].append(float(np.mean(k_layer_vals)))
        # q_layer_vals = torch.stack(q_layer_vals).flatten().cpu().numpy()
        # k_layer_vals = torch.stack(k_layer_vals).flatten().cpu().numpy()
        # q_norms_all[n].append(q_layer_vals)
        # k_norms_all[n].append(k_layer_vals)
...
# ---------------------------------------------------------------------------
# q_means = [float(np.mean(q_norms_all[n])) for n in POWERS_OF_TWO]
# k_means = [float(np.mean(k_norms_all[n])) for n in POWERS_OF_TWO]
# q_vars  = [float(np.var(q_norms_all[n]))  for n in POWERS_OF_TWO]
# k_vars  = [float(np.var(k_norms_all[n]))  for n in POWERS_OF_TWO]
gamma_n = [float(np.mean(gamma_all[n])) for n in POWERS_OF_TWO]

# print("\n Results:")
# print(f"{'n':>6}  {'q_mean':>10}  {'k_mean':>10}  {'q_var':>10}  {'k_var':>10}")
# for n, qm, km, qv, kv in zip(POWERS_OF_TWO, q_means, k_means, q_vars, k_vars):
#     print(f"{n:>6}  {qm:>10.4f}  {km:>10.4f}  {qv:>10.4f}  {kv:>10.4f}")
# print("\n| n | q_mean | k_mean | q_std | k_std |")
# print("|--:|-------:|-------:|------:|------:|")
# for n, qm, km, qv, kv in zip(POWERS_OF_TWO, q_means, k_means, q_vars, k_vars):
#     print(f"| {n} | {qm:.4f} | {km:.4f} | {qv**0.5:.4f} | {kv**0.5:.4f} |")
print("|--:|-------:|")
...
# ax.plot(POWERS_OF_TWO, q_means, marker="o", linewidth=2, label="Query max norm")
# ax.plot(POWERS_OF_TWO, k_means, marker="s", linewidth=2, label="Key max norm")
ax.plot(POWERS_OF_TWO, [np.mean(gamma_all[n]) for n in POWERS_OF_TWO], marker="o", linewidth=2, label="Gamma (scaled qk norm)")
# ax.fill_between(POWERS_OF_TWO,
#     [m - v**0.5 for m, v in zip(q_means, q_vars)],
#     [m + v**0.5 for m, v in zip(q_means, q_vars)],
#     alpha=0.2)
# ax.fill_between(POWERS_OF_TWO,
#     [m - v**0.5 for m, v in zip(k_means, k_vars)],
#     [m + v**0.5 for m, v in zip(k_means, k_vars)],
#     alpha=0.2)

ax.set_xscale("log", base=2)
ax.set_xticks(POWERS_OF_TWO)
ax.set_xticklabels([str(n) for n in POWERS_OF_TWO], rotation=45, ha="right")
ax.set_xlabel("Sequence length n  (log₂ scale)")
ax.set_ylabel("Average maximum norm")
ax.set_title(
    f"$\gamma(n)$ averaged over all attention layer heads and 10 samples\n"
    f"(Qwen2.5-7B-Instruct, {DATASET_NAME}, {DATASET_CONFIG} dataset)"
)
ax.legend()
